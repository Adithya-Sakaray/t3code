...
**Usage** combines Codex, Claude Code, Grok Build, and Kiro CLI session history from your
...
Kiro CLI bills in credits; the page converts those credits at the public overage rate
($0.04 per credit). Kiro often leaves token counters at zero, so Kiro rows may show cost with
little or no token volume.
