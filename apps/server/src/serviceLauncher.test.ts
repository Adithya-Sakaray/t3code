...
// A pinned runtime is the npm CLI entry at <versionDir>/node_modules/t3/dist/bin.mjs.
const writeFakeRuntime = (
  ...
  Effect.gen(function* () {
    const { entryPath, sentinelPath } = runtimePaths(baseDir, version);
    ...
    yield* fs.writeFileString(sentinelPath, `${version}\n`);
    return entryPath;
  });
...
it.layer(NodeServices.layer)("service state persistence", (it) => {
  ...
  it.effect("a fresh launcher clears a restart deferred by t3 update", () =>
    Effect.gen(function* () {
      ...
      yield* writeFakeRuntime(fs, path, root, "1.0.0", "setInterval(() => {}, 1_000);\n");
...
`;
      for (const version of ["1.0.0", "1.1.0"]) {
        yield* writeFakeRuntime(fs, path, root, version, childSource);
      }
      yield* Effect.promise(() =>
        writeServiceState(statePath, {
          protocol: SERVICE_LAUNCHER_PROTOCOL,
