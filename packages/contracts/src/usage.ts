 ...
 * `~/.grok/sessions/**\/updates.jsonl`, and `~/.kiro/sessions/cli/*.json`
 * metering sidecars) rather than relying on T3 Code's own orchestration
 ...
 *
 * v5 adds `grok` and this fork's `kiro` to {@link UsageProviderKind}; v4
...
export const UsageProviderKind = Schema.Literals(["claude", "codex", "grok", "kiro"]);
